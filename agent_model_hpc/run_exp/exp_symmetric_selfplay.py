...
usage: python -m exp_symmetric_selfplay -j __STR__ -w __STR__ -s allc -g pd -r scalar -e 10 -t 100 -p na -k true -z false -c false -l true
    
    localhost example: 
        python -m  run_exp.exp_symmetric_selfplay -j '1111.localhost' -w 'localhost' -s allc  -g pd -r scalar -e 10 -t 100 -p 'na' -k true -z false -c false -l true
    or, 
        python -m  run_exp.exp_symmetric_selfplay -j '1111.localhost' -w 'localhost' -s actor_critic_1ed  -g pd -r scalar -e 10 -t 100 -p 'alpha=0.1:gamma=0.2' -k true -z false -c false -l true
...
def main(argv):
    ...
    exp_symmetric_selfplay(exp_data)
...
def exp_symmetric_selfplay(exp_data):
    ...
    strategy_parameters = exp_utility().load_symmetric_strategy_parameters(exp_data, matrix)
...
def parse_input(exp_data, argv):
    ...
    try:
        options, args = getopt.getopt(argv, "hj:w:s:f:g:r:e:t:p:k:z:c:l:", ["pbs_jobstr", "pbs_scratch_dir", "strategy", "family", "gameform", "reward_type", "episodes", "timesteps", "parameter_string", "write_state", "compress_writes", "write_2_heartbeats", "localhost"])
