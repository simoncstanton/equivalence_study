#!/usr/bin/env python3
# File: bandit_noninc_softmax_ap_2ed.py
...
class Bandit_noninc_softmax_ap_2ed(Strategy):
    ...
    def action(self, agent, previous_step) -> int:
        ...
        if len(agent.action_history) == 0:
            ...
            self.av_r = sum(self.r_k_0) + sum(self.r_k_1) / (len(agent.action_history) + 1)
    ...
    def get_strategy_internal_state(self):
        return { "a_p" : self.a_p.copy(), "h_a_0" : self.h_a_0, "h_a_1" : self.h_a_1, "av_r" : self.av_r, "r_k_0": self.r_k_0.copy(), "r_k_1": self.r_k_1.copy() }
