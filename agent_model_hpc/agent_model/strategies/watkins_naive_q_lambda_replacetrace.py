#!/usr/bin/env python3
# File: watkins_naive_q_lambda_replacetrace.py
...
class Watkins_naive_q_lambda_replacetrace(Strategy):
    ...
    def __init__(self, strategy, strategy_options):
        ...
        '''
        RL method
        Watkins Naive Q-lambda, Sutton&Barto 1st edition
            No penalty to eligibility traces from exploratory actions`
    ...
    def action(self, agent, previous_step) -> int:
        ...
        if len(agent.action_history) == 0:
            ...
            ''' find a*
                ...
                may not be if rnd < epsilon 
            ''' 
            optimal_action = self.Q[self.state].index(maxq)
            if action == optimal_action:
                optimal_action = action
            
            ''' calc td error '''
            td_error = r + (self.options["gamma"] * self.Q[self.state][optimal_action]) - self.Q[self.prev_state][prev_action]

            
            ''' update eligibility trace for previous state'''
            # not required for replace-trace
            # self.trace[self.prev_state][prev_action] += 1
            
            ''' update Q and trace for all states '''
            for i, s in enumerate(self.Q):
                for j, a in enumerate(s):
                    # no penalty for exploring, normal decay on trace
