#!/usr/bin/env python3
# File: actor_critic_1ed_replacetrace.py
...
class Actor_critic_1ed_replacetrace(Strategy):
    ...
    def __init__(self, strategy, strategy_options):
        ...
        RL method
            ...
            with replacing traces on Actor
        ...
        # 1 initialise p_a, V
        # self.smv = OrderedDict({"": [0.5,0.5]})
        # self.p_a = OrderedDict({"": [0,0]})
        # self.V = OrderedDict({"": 0})  # doesn't need to be Ordered [TODO]
        
        # self.actor_trace = OrderedDict({"": [0, 0]}) # need not be ordered [TODO]
        # self.critic_trace = OrderedDict({"": 0}) # need not be ordered [TODO]
            ...
            for i, s in enumerate(self.actor_trace):
                for j, a in enumerate(s):
                    ...
                    if s == self.prev_state and int(a) == prev_action:
                        self.actor_trace[s][int(a)] = 1
                    elif s == self.prev_state:
                        self.actor_trace[s][int(a)] = 0
                    else:
